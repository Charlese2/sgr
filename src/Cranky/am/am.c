...
u32 AMPush(char *path) {
    ...
    if (buffer_length) {
        ...
        ret_val = __AMPushBuffered(path, buffer, offset, buffer_length, NULL, FALSE);
        OSFree(buffer);
        return ret_val;
    } else {
#ifdef DEBUG
...
u32 AMPushData(void *buffer, u32 length) {
    ...
    if (__AMFreeBytes >= round_length && __AMStackLocation < AM_STACK_ENTRIES - 1) {
        ...
        ARQPostRequest(&ptr->arq_handle, __AMStackLocation - 1, 0, 1, (u32)buffer, ptr->aram_start_addr, round_length,
...
u32 __AMPushBuffered(char *path, void *buffer, u32 offset, u32 buffer_size, AMCallback callback, int async_flag) {
    ...
    if (__AMStackLocation < AM_STACK_ENTRIES - 1) {
        ...
        if (!DVDOpen(path, &ptr->file_handle)) {
            sprintf(error_msg_buffer, "AMPushBuffered(): Unable to open file '%s'\n", path);
            OSReport(error_msg_buffer);
            OSPanic(__FILE__, 509, "AM: FATAL ERROR\n");
        }
        ...
        if (__AMFreeBytes >= round_file_length) {
            ...
            if (async_flag == 1) {
                DVDReadAsync(&ptr->file_handle, ptr->buffer, ptr->read_length, ptr->curr_read_offset, __AM_dvd_callback);
                __AMPendingReads++;
            } else {
                while (ptr->curr_read_offset < ptr->file_length) {
                    ...
                    ARQPostRequest(&(ptr->arq_handle), stack_index, 0, 1, (u32)ptr->buffer, ptr->curr_aram_offset, actual_read_length,
                                   __AM_arq_poll_callback);
...
u32 AMGetZeroBuffer(void) { return __AMStackPointer[0]; }
